...
public enum CardData {
    ATTACK("Attack", "pick a player, he has to play defend or get 1 damage", CardType.BASICCARD,
        new Effect<>(new Function<CardUseHandle, Boolean>() {
            @Override
            public Boolean apply(CardUseHandle handle) {
                ...
                for (Player target : handle.getGameManager().getGame().getOtherPlayers(player)) {
                    if (!target.equals(player) && target.isAlive &&
                        Boolean.FALSE.equals(target.getImmunity().get(ATTACK))) {
                        targets.add(target);
                    }
                }
            ...
            public Boolean apply(CardUseHandle cardUseHandle) {
                ...
                ArrayList<Player> pickTarget = gameManager.getGameController()
                    .askForChosePlayer(player, targets, 1, 1, true, "pick a player to attack");
                ...
                for (Player target : pickTarget) {
                    AttackHandle attackHandle =
                        new AttackHandle(gameManager, cardUseHandle.getCard(), "", player, target, 0);
                    ...
                    if (gameManager.getGameController().askForDiscard(cardAskHandle)) {
                        ...
                        DamageHandle damageHandle =
                            new DamageHandle(cardUseHandle.getGameManager(), cardUseHandle.getCard(), "attack damaged",
                                player, target, 1 + attackHandle.getExtraDamage(), DamageType.NORMAL);
        ...
        })), DEFEND("Defend", "use when you are getting Attack, prevent the damage of Attack", CardType.BASICCARD,
        new Effect<>(false, false));
